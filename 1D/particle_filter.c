...
ParticleFilter* pf_create(int n_particles, int n_regimes) {
    ...
    if (pf->n_threads > PF_MAX_THREADS) pf->n_threads = PF_MAX_THREADS;
...
void pf_destroy(ParticleFilter* pf) {
    ...
    for (int t = 0; t < pf->n_threads; t++) {
        if (pf->rng[t]) vslDeleteStream(&pf->rng[t]);
    }
    
    if (pf->states)      mkl_free(pf->states);
    if (pf->states_tmp)  mkl_free(pf->states_tmp);
    if (pf->weights)     mkl_free(pf->weights);
    if (pf->log_weights) mkl_free(pf->log_weights);
    if (pf->cumsum)      mkl_free(pf->cumsum);
    if (pf->noise)       mkl_free(pf->noise);
    if (pf->uniform)     mkl_free(pf->uniform);
    if (pf->scratch)     mkl_free(pf->scratch);
    if (pf->regimes)     mkl_free(pf->regimes);
    if (pf->regimes_tmp) mkl_free(pf->regimes_tmp);
...
void pf_set_regime_params(ParticleFilter* pf, int regime,
    ...
    if (regime < 0 || regime >= PF_MAX_REGIMES) return;
