...
static double scalar_pmmh_log_lik(const double *ret, int n, int np,
    ...
    for (int i = 0; i < np; i++) lv[i] = p->mu_vol + scalar_randn() * 0.3;
...
static void scalar_pmmh_run(const double *ret, int n, int np,
    ...
    for (int it = 0; it < n_iter; it++) {
        PMMHParams prop;
        prop.drift = cur.drift + scalar_randn() * 0.0004;
        prop.mu_vol = cur.mu_vol + scalar_randn() * 0.08;
        prop.sigma_vol = cur.sigma_vol * exp(scalar_randn() * 0.05);
...
int main(int argc, char **argv) {
    ...
    int n_iter = 300;
    int n_burn = 100;
    int n_runs = 5;  /* Runs per configuration */
