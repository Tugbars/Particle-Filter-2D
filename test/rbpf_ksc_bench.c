...
SyntheticData* generate_synthetic_data(int n, int seed) {
    ...
    for (int t = 0; t < n; t++) {
        ...
        if (t == n / 5) regime = 1;           /* 20%: enter medium vol */
        if (t == 2 * n / 5) regime = 2;       /* 40%: crisis begins */
        if (t == n / 2) regime = 3;           /* 50%: peak crisis */
        if (t == 3 * n / 5) regime = 2;       /* 60%: crisis easing */
        if (t == 7 * n / 10) regime = 1;      /* 70%: recovery */
        if (t == 4 * n / 5) regime = 0;       /* 80%: back to calm */
...
AccuracyMetrics compute_accuracy(const float *est_vol, const float *est_log_vol,
    ...
    for (int t = 0; t < n; t++) {
        ...
        if (data->true_regime[t] == est_regime[t]) regime_correct++;
...
int main(int argc, char **argv) {
    ...
    for (int t = 1; t < n_obs; t++) {
        if (data->true_vol[t] < min_vol) min_vol = data->true_vol[t];
        if (data->true_vol[t] > max_vol) max_vol = data->true_vol[t];
    }
