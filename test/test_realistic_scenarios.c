 ...
 * CONFIGURATION
 *
 * For quick CI tests: N_MONTE_CARLO=20, particles=128
 * For high-quality: N_MONTE_CARLO=50, particles=256 (use MKL build)
 *============================================================================*/
...
#define PMMH_ITERATIONS 250
#define PMMH_BURNIN 80
                    ...
                    const SVParams *p, double *last_return)
{
    ...
    /* Vol dynamics */
    double new_log_vol = (1.0 - p->theta_vol) * (*log_vol) + p->theta_vol * p->mu_vol + p->sigma_vol * z2;
                                   ...
                                   double *returns, int n)
{
    ...
    for (int t = 1; t < n; t++)
    {
        ...
        if (t == 1)
            prices[1] = prices[0] * (1.0 + returns[0]);
    }
    returns[n - 1] = 0.0;
}
...
typedef struct
{
    ...
    int alert_count;         /* Stage 1: mild threshold breaches */
    int confirm_count;       /* Stage 2: strong threshold breaches */
    double mild_threshold;   /* Lower threshold for alert */
    double strong_threshold; /* Higher threshold for confirmation */
    int alert_required;      /* Alerts needed before checking confirm */
    int confirm_required;    /* Confirmations needed to trigger */

    /* Hysteresis: don't re-trigger within cooldown */
    int cooldown;
    int last_trigger;
} Detector;
...
static Detector *detector_create(int window, int warmup)
{
    ...
    /* Default thresholds - tuned for good FP/FN tradeoff */
    d->mild_threshold = 1.8;
    d->strong_threshold = 2.5;
    d->alert_required = 3;
    d->confirm_required = 2;
    d->cooldown = 50;
    d->last_trigger = -1000;
...
/* Returns 1 if confirmed detection, 0 otherwise */
static int detector_update(Detector *d, double x, int t, double *ratio_out)
{
    d->buffer[d->head] = x;
    ...
    if (!d->baseline_set && t >= d->warmup_end)
    {
        ...
        d->baseline_set = 1;
        if (d->baseline_var < 1e-12)
            d->baseline_var = 1e-12;
    }
    ...
    double ratio = current_var / d->baseline_var;
    if (ratio_out)
        *ratio_out = ratio;

    /* Cooldown check */
    if (t - d->last_trigger < d->cooldown)
        return 0;

    /* Two-stage logic */
    if (ratio > d->mild_threshold)
    {
        d->alert_count++;
        if (ratio > d->strong_threshold)
        {
            d->confirm_count++;
        }
    }
    ...
    {
        /* Reset if ratio drops below mild threshold */
        d->alert_count = 0;
        d->confirm_count = 0;
    }

    /* Trigger if: enough alerts AND at least some strong confirmations */
    if (d->alert_count >= d->alert_required && d->confirm_count >= d->confirm_required)
    {
        d->last_trigger = t;
        d->alert_count = 0;
        d->confirm_count = 0;
...
static double pmmh_log_lik(const double *ret, int n, const PMMHParams *p, double theta)
{
    ...
    /* Initialize particles spread around mu_vol */
    for (int i = 0; i < np; i++)
        lv[i] = p->mu_vol + randn() * 0.3;
...
static void run_scenario_monte_carlo(const Scenario *s, int n_runs, MCStats *stats)
{
    ...
    for (int run = 0; run < n_runs; run++)
    {
        ...
        {
            double ratio;
            int detected = detector_update(det, returns[t], t, &ratio);
            if (detection_time < 0 && detected && t > warmup + 50)
            {
                detection_time = t;
            }
        }
