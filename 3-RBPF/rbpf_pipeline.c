...
RBPF_PipelineConfig rbpf_pipeline_default_config(void) {
    ...
    cfg.theta[0] = RBPF_REAL(0.05);  cfg.mu_vol[0] = RBPF_REAL(-4.6);  cfg.sigma_vol[0] = RBPF_REAL(0.05);
    cfg.theta[1] = RBPF_REAL(0.08);  cfg.mu_vol[1] = RBPF_REAL(-3.5);  cfg.sigma_vol[1] = RBPF_REAL(0.10);
    cfg.theta[2] = RBPF_REAL(0.12);  cfg.mu_vol[2] = RBPF_REAL(-2.5);  cfg.sigma_vol[2] = RBPF_REAL(0.20);
    cfg.theta[3] = RBPF_REAL(0.15);  cfg.mu_vol[3] = RBPF_REAL(-1.6);  cfg.sigma_vol[3] = RBPF_REAL(0.30);
    ...
    /* Change detection */
    cfg.surprise_minor = RBPF_REAL(3.0);
    cfg.surprise_major = RBPF_REAL(5.0);
    cfg.vol_ratio_minor = RBPF_REAL(1.5);
    cfg.vol_ratio_major = RBPF_REAL(2.0);
...
void rbpf_pipeline_destroy(RBPF_Pipeline *pipe) {
    if (!pipe) return;
    if (pipe->rbpf) rbpf_ksc_destroy(pipe->rbpf);
    free(pipe);
}
...
void rbpf_pipeline_step(RBPF_Pipeline *pipe, rbpf_real_t ssa_return, RBPF_Signal *sig) {
    ...
    if (out.regime_changed || (pipe->ticks_in_regime == 1 && pipe->tick_count > 1)) {
        regime_shift = 1;
        if (sig->change_detected < 1) sig->change_detected = 1;
    }
...
void rbpf_pipeline_step_apf(RBPF_Pipeline *pipe, 
    ...
    sig->vol_forecast = out.vol_mean;
    sig->log_vol = out.log_vol_mean;
    sig->vol_uncertainty = out.smooth_valid ? 
                           rbpf_sqrt(out.log_vol_var_smooth) : 
                           rbpf_sqrt(out.log_vol_var);
